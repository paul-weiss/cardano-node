...
$SED -i "${ROOT}/configuration.yaml" \
  ...
  # We want to hardfork to PV 9 via update proposal (i.e exit the bootstrap era)
...
# Bootstrap era  
# - DReps can only vote on info actions
# - SPOs and Constitutional Committee members can vote on allowed GovActions
# - The only allowed bootstrap actions are ParameterChange, HardForkInitiation, Info Action
# - All other actions are not allowed.
# We need to seed the genesis with a constitutional committee and then SPOs and CC members 
# need to vote on the hardfork gov action to hardfork to PV 10
# format: "committee":{"members":{"keyHash-bf883531c456f3868304a7e6613bcd832da7ea74e300812562503332":100}

committeedir="${ROOT}/committee"
mkdir -p "${committeedir}"

# Loop from 1 to 5 (change as needed)
for i in {1..2}
do
    # Committee cold vkey
    committee_vkeyfp="$committeedir/committee_cold_vkey_$i"
    # Committee cold skey 
    committee_skeyfp="$committeedir/committee_cold_skey_$i"

    # Generate committee cold keys
    cardano-cli conway governance committee\
      key-gen-cold\
      --cold-verification-key-file "$committee_vkeyfp"\
      --cold-signing-key-file "$committee_skeyfp" 
      
    committeevkeyhash="keyHash-$(cardano-cli conway governance committee key-hash --verification-key-file "$committee_vkeyfp")"
    
    conwaygenesisfp="${ROOT}/genesis/shelley/genesis.conway.json"

    # Read the current JSON object from file
    current_conaygenesis=$(cat "$conwaygenesisfp")

    committeememberexpiry=100
    # Append new data to the JSON object under the "members" key within the "committee"
    updated_json=$(jq --arg key "$committeevkeyhash" --argjson value "$committeememberexpiry" \
                      '.committee.members[$key] = $value' <<< "$current_conaygenesis")
    
    # Write the updated JSON object back to file
    echo "$updated_json" > "$conwaygenesisfp"
done
